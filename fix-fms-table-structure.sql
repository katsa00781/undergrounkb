-- Update FMS assessments table to match the correct schema structure
-- This script will recreate the table with proper left/right columns
...
-- Create FMS assessments table with correct structure
CREATE TABLE public.fms_assessments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  deep_squat INTEGER NOT NULL CHECK (deep_squat BETWEEN 0 AND 3),
  hurdle_step_left INTEGER NOT NULL CHECK (hurdle_step_left BETWEEN 0 AND 3),
  hurdle_step_right INTEGER NOT NULL CHECK (hurdle_step_right BETWEEN 0 AND 3),
  inline_lunge_left INTEGER NOT NULL CHECK (inline_lunge_left BETWEEN 0 AND 3),
  inline_lunge_right INTEGER NOT NULL CHECK (inline_lunge_right BETWEEN 0 AND 3),
  shoulder_mobility_left INTEGER NOT NULL CHECK (shoulder_mobility_left BETWEEN 0 AND 3),
  shoulder_mobility_right INTEGER NOT NULL CHECK (shoulder_mobility_right BETWEEN 0 AND 3),
  active_straight_leg_left INTEGER NOT NULL CHECK (active_straight_leg_left BETWEEN 0 AND 3),
  active_straight_leg_right INTEGER NOT NULL CHECK (active_straight_leg_right BETWEEN 0 AND 3),
  trunk_stability_pushup INTEGER NOT NULL CHECK (trunk_stability_pushup BETWEEN 0 AND 3),
  rotary_stability_left INTEGER NOT NULL CHECK (rotary_stability_left BETWEEN 0 AND 3),
  rotary_stability_right INTEGER NOT NULL CHECK (rotary_stability_right BETWEEN 0 AND 3),
  shoulder_clearing_left BOOLEAN DEFAULT FALSE,
  shoulder_clearing_right BOOLEAN DEFAULT FALSE,
...
COMMENT ON TABLE public.fms_assessments IS 'FMS (Functional Movement Screen) assessment results with proper left/right scoring';
